...
public record StatusLine(StatusCode status, HttpVersion version, String rawValue) {
    ...
    public static StatusLine extractStatusLine(String value) {
        ...
        assert mr.matches();
        // String.format("%s must match the statusLinePattern: %s", value, statusLinePattern))
        ...
        HttpVersion httpVersion = switch (version) {
            ...
            default ->
                    throw new RuntimeException(String.format("HTTP version was not an acceptable value. Given: %s", version));
        };
